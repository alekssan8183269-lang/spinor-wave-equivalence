...
*   `∑_sin` (W_SUM_SIN) — Волновой Интерференционный Аккумулятор (Волновая Сумма): 
...
*   `∪_complex` (M_UNION_COMPLEX) — Резонансное Комплексное Объединение множеств:
...
*   `∩_tan` (M_INTERSECT_TAN) — Тангенциальное пересечение контекстов:
...
*   `{≻_matrix}` (SYS_PERTURB_STORM) — Синусоидальная Система Неравенств (Ловушка Хаоса):
